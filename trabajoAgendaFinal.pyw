...
def crear_interfaz_notas():
    ...
    def mostrar_contenido_seleccionado(event):
        selected_index = listbox.curselection() #Informacion seleccionado
        if selected_index:
    ...
    listbox.pack(side=tk.RIGHT, padx=10, pady=10)
    ...
    tk.Button(marco_campos, text="Volver al menú", command=mostrar_menu_principal).grid(row=4, column=2, padx=5)
...
def crear_interfaz_tareas():
        ...
        def mostrar_tareas_completadas():
            if not tareas_completadas:
                messagebox.showinfo("No hay tareas completadas")
            else:
                tareas_completadas_texto = "\n".join([f"{i+1}. {tarea['Titulo']} - Fecha de vencimiento: {tarea['Fecha']} - Prioridad: {tarea['Prioridad']}" for i, tarea in enumerate(tareas_completadas)])
                messagebox.showinfo("Tareas completadas", tareas_completadas_texto)
        ...
        # titulo de la listbox
        titulo_label = tk.Label(frame1, text="Tareas Pendientes")
        ...
        input_frame.pack(before=titulo_label, padx=10, pady=10)
        ...
        tk.Button(botones_frame, text="Mostrar Tareas Completadas", command=mostrar_tareas_completadas).grid(row=0, column=2, padx=5)
        tk.Button(botones_frame, text="Editar Tarea", command=editar_tarea).grid(row=0, column=3, padx=5)
        tk.Button(botones_frame, text="Marcar como completada", command=marcar_completada).grid(row=0, column=4, padx=5)
        tk.Button(botones_frame, text="Volver al menú", command=mostrar_menu_principal).grid(row=1, column=1, padx=5)
...
def abrir_interfaz_eventos():
    ...
    tk.Button(app, text="Volver al menú", command=mostrar_menu_principal).grid(row=8, column=3, padx=5)
